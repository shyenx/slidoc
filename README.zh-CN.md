...
视频是最自然的讲解 / 演示 / 讨论载体——但它**事后回看效率极低**。

- **不能跳读。** 一段两小时的讲座要花两小时看，没有 Ctrl-F。
- **不能采样。** 拉动时间轴根本看不出哪一段内容密度高。
- **不能复用。** 想引用一句话、一张幻灯片、一个数字？回到视频里翻。
- **不能批量。** 十段培训视频里反复讲同一套方法论，淹没在 15 小时录像里。

读文字比看视频快 5-10 倍。**「PPT 缩略图 + 清洗后的讲稿」给你一份可扫读、可搜索、可摘录、可复用的文档**，而每段都对应回原视频的具体时间点，主讲人的原话基本保留。

`slidoc` 解决三种具体场景：

1. **「一段长视频，我只有 30 分钟。」** 读文档而不是看视频，跳到关心的那张 PPT 即可。
2. **「我现场听过，但没记笔记。」** 比你自己速记更快地拿到一份「带 PPT 的逐段讲稿」。
3. **「我有一整个系列。」** 一次性处理 5+ 个视频，把「一堆 mp4」变成「一份可搜索的资料库」。
